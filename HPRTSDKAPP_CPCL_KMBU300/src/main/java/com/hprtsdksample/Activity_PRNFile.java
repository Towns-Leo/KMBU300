  		...
  		{  		  			
	  		switch(resultCode)
	  		{
	  			case KMPrinterHelper.ACTIVITY_IMAGE_FILE:	  				
					setResult(KMPrinterHelper.ACTIVITY_IMAGE_FILE, data);
	  				this.finish();
	  			case KMPrinterHelper.ACTIVITY_PRNFILE:
	  				setResult(KMPrinterHelper.ACTIVITY_PRNFILE, data);
	  				this.finish();	  			
	  		}
  		}
  		catch(Exception e)
	...
	{		
		...
		{				
			...
			{			
				...
				if(file.isFile())
				{
					int ResultType=(Invalid(file)==0?KMPrinterHelper.ACTIVITY_PRNFILE:KMPrinterHelper.ACTIVITY_IMAGE_FILE);
