...
//		iStatusMode=this.getIntent().getIntExtra("StatusMode", 1);
	...
	private void Refresh(){
		txtStatus.setText("");
		try {
			int getstatus = KMPrinterHelper.getstatus();
			switch (getstatus) {
			case 0:
	...
	/*private void Refresh()
	{
		try
		{
			...
			if(iStatusMode == 1)	//only transmit status
			{
				...
				iReturn=KMPrinterHelper.GetTransmitStatus(1, statusData);
				if(iReturn>0)
				{
					if(statusData[0]==0)
			...
			{
				statusData=new byte[1];
				iReturn=KMPrinterHelper.GetTransmitState(32, statusData);
				if(iReturn>0)
				{
					if(statusData[0]==0)
						sStatus=thisCon.getString(R.string.activity_status_transmit_status_no_paper);
			...
			if((iStatusMode & 2) > 0)	//have real time status,needn't transmit status
			{
				//get paper status
				statusData=new byte[1];
				iReturn=KMPrinterHelper.GetRealTimeStatus((byte)KMPrinterHelper.PRINTER_REAL_TIME_STATUS_ITEM_PAPER, statusData);
				if(iReturn<=0)
				{
				...
				statusData=new byte[1];
				iReturn=KMPrinterHelper.GetRealTimeStatus((byte)KMPrinterHelper.PRINTER_REAL_TIME_STATUS_ITEM_PRINTER, statusData);
				if(iReturn<=0)
				{
				...
				if((statusData[0] & 8)>0)
				{
					//get off line status
					statusData=new byte[1];
					iReturn=KMPrinterHelper.GetRealTimeStatus((byte)KMPrinterHelper.PRINTER_REAL_TIME_STATUS_ITEM_ONOFFLINE, statusData);
					if(iReturn<=0)
					{
				...
				//get error status
				statusData=new byte[1];
				iReturn=KMPrinterHelper.GetRealTimeStatus((byte)KMPrinterHelper.PRINTER_REAL_TIME_STATUS_ITEM_ERROR, statusData);
				if(iReturn<0)
				{
